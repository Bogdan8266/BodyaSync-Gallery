...
Future<List<MemoryStory>> fetchMemoryStories() async {
    ...
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(utf8.decode(response.bodyBytes));
      return data.map((storyJson) => MemoryStory.fromJson(storyJson)).toList();
    } else {
      throw Exception('Failed to load memories');
    }
  }

  Future<String> startMemoryGeneration() async {
    final baseUrl = await getBaseUrl();
    if (baseUrl.isEmpty) throw Exception('IP не налаштовано');
    final response = await http.post(Uri.parse('$baseUrl/memories/generate'));
    if (response.statusCode == 200) {
      return json.decode(response.body)['task_id'];
    } else {
  ...
  Future<Map<String, dynamic>> checkGenerationStatus(String taskId) async {
    final baseUrl = await getBaseUrl();
    if (baseUrl.isEmpty) throw Exception('IP не налаштовано');
    final response = await http.get(Uri.parse('$baseUrl/memories/status/$taskId'));
    if (response.statusCode == 200) {
...
class _MainScreenState extends State<MainScreen> with SingleTickerProviderStateMixin {
  ...
  // <--- ЗМІНА: Логіка блюру переїхала сюди, щоб покривати весь екран
  ...
  @override
  Widget build(BuildContext context) {
    // Віджети для кожної вкладки
    final List<Widget> pages = [
      GalleryScreen(
        key: const ValueKey('gallery_page'),
        blurAnimationController: _blurAnimationController, // Передаємо контролер вниз
      ),
    ...
    return Scaffold(
      ...
      extendBody: true,
      // <--- ЗМІНА: AppBar тепер в NestedScrollView, тому тут його немає
      body: Stack(
        children: [
          // Використовуємо NestedScrollView для екрану Галереї,
          // і звичайний PageTransitionSwitcher для інших екранів.
          _selectedIndex == 0
              ? NestedScrollView(
                  headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
                    return <Widget>[
                      SliverAppBar(
                        // <--- НОВЕ: Це і є наш ховаючийся AppBar
                        title: Text('BodyaSync', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                        leading: IconButton(icon: const Icon(Icons.person_outline), onPressed: () {}),
                        actions: [
                          IconButton(icon: const Icon(Icons.auto_awesome_outlined), onPressed: () {}),
                          IconButton(icon: const Icon(Icons.search), onPressed: () {}),
                          IconButton(icon: const Icon(Icons.settings_outlined), onPressed: () => _onItemTapped(2)),
                        ],
                        // Властивості для "ховающегося" ефекту
                        floating: true,
                        snap: true,
                  ...
                  body: pages[0], // Тіло - це наш GalleryScreen
                )
          ...
          // Шар з блюром (тепер він на найвищому рівні)
          AnimatedBuilder(
            animation: _blurAnimation,
            builder: (context, child) {
              if (_blurAnimation.value == 0.0) return const SizedBox.shrink();
              return IgnorePointer(
                ignoring: _blurAnimationController.status == AnimationStatus.reverse,
                child: BackdropFilter(
                  filter: ImageFilter.blur(sigmaX: 5 * _blurAnimation.value, sigmaY: 5 * _blurAnimation.value),
                  child: Container(color: Colors.black.withOpacity(0.4 * _blurAnimation.value)),
                ),
              );
            },
          ),
...
class MemoryItem {
  final String label;
  final String date;
  final String imageUrl; // Будемо використовувати для фону
  
  MemoryItem({required this.label, required this.date, required this.imageUrl});
}


class GalleryScreen extends StatefulWidget {
  // <--- ЗМІНА: Приймаємо контролер блюру ззовні
  final AnimationController blurAnimationController;

  const GalleryScreen({
    Key? key,
    required this.blurAnimationController,
  }) : super(key: key);
...
class StoryPage {
  final String type; // 'photo' or 'collage'
  ...
  StoryPage({
    ...
    this.duration = const Duration(seconds: 5), // Стандартно 5 секунд
  });

  factory StoryPage.fromJson(Map<String, dynamic> json, String baseUrl) {
    return StoryPage(
      type: json['type'],
...
// Модель для цілої історії-спогаду


// main.dart

// <--- НОВІ МОДЕЛІ ДЛЯ СПОГАДІВ
class StoryItem {
  final String type; // 'image' or 'collage'
  final String imageUrl;
  final String caption;

  StoryItem({required this.type, required this.imageUrl, required this.caption});
...
class MemoryStory {
  ...
  factory MemoryStory.fromJson(Map<String, dynamic> json) {
    var itemsFromJson = json['items'] as List;
    List<StoryItem> storyItems = itemsFromJson.map((i) => StoryItem.fromJson(i)).toList();
    return MemoryStory(
      id: json['id'],
      title: json['title'],
  ...
  return RefreshIndicator(
    ...
    child: FutureBuilder<List<GalleryItem>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return Center(child: Padding(padding: const EdgeInsets.all(16.0), child: Text('Помилка: ${snapshot.error}', textAlign: TextAlign.center)));
        }
...
class _StoryViewerScreenState extends State<StoryViewerScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // <--- ЗМІНА 1: Ми більше не намагаємося перемкнути сторінку тут.
    // Ми лише запускаємо логіку анімації для першого елемента.
    _startStoryAnimation(0);

    // <--- ЗМІНА 2: Використовуємо 'addPostFrameCallback', щоб безпечно перейти на першу сторінку
    // ПІСЛЯ того, як віджет буде побудовано.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        _pageController.jumpToPage(0); // Стрибаємо на першу сторінку
        
        // Запускаємо музику тільки після того, як все готово
        if (widget.story.musicUrl != null) {
          apiService.getBaseUrl().then((baseUrl) {
            if(baseUrl.isNotEmpty && mounted) {
              final fullMusicUrl = baseUrl + widget.story.musicUrl!;
              _audioPlayer.play(UrlSource(fullMusicUrl));
            }
          });
        }
      }
  ...
  // <--- ЗМІНА 3: Спрощуємо функцію, тепер вона керує тільки анімацією
  void _startStoryAnimation(int index) {
    _animController.stop();
    _animController.reset();
    _animController.duration = const Duration(seconds: 7);
    _animController.forward();
    
    // Видаляємо старий слухач, щоб уникнути дублювання, і додаємо новий
    _animController.removeStatusListener(_onAnimationCompleted);
    _animController.addStatusListener(_onAnimationCompleted);

    if(mounted) setState(() => _currentIndex = index);
  }
  
  // <--- НОВЕ: Виносимо логіку завершення анімації в окремий метод
  ...
  void _nextPage() {
    if (_currentIndex < widget.story.items.length - 1) {
      ...
      _pageController.animateToPage(nextIndex, duration: const Duration(milliseconds: 300), curve: Curves.easeIn);
      _startStoryAnimation(nextIndex);
    } else {
  ...
  void _previousPage() {
    if (_currentIndex > 0) {
      ...
      _pageController.animateToPage(prevIndex, duration: const Duration(milliseconds: 300), curve: Curves.easeIn);
      _startStoryAnimation(prevIndex);
    } else {
      // Перезапускаємо анімацію для першої сторінки
      _startStoryAnimation(0);
    }
  }
  ...
  @override
  Widget build(BuildContext context) {
    // Решта методу build залишається такою ж, як у попередній версії,
    // оскільки вона була правильною.
    final storyItems = widget.story.items;
    return Scaffold(
      backgroundColor: Colors.black,
        ...
        child: Stack(
          children: [
            PageView.builder(
              ...
              // <--- ЗМІНА 4: Слухаємо, коли користувач вручну змінює сторінку (якщо б скрол був увімкнений)
              // Це не обов'язково зараз, але корисно для майбутнього.
              onPageChanged: (index) {
                // Синхронізуємо анімацію, якщо сторінка змінилась
                if (index != _currentIndex) {
                  _startStoryAnimation(index);
                }
              },
              itemBuilder: (context, index) {
                ...
                return FutureBuilder<String>(
                  ...
                  builder: (context, urlSnapshot) {
                    ...
                    return Stack(
                      ...
                      children: [
                        CachedNetworkImage(
                          imageUrl: urlSnapshot.data! + item.imageUrl,
                          fit: BoxFit.contain,
                          placeholder: (c, u) => const Center(child: CircularProgressIndicator(color: Colors.white24)),
                          errorWidget: (c, u, e) => const Center(child: Icon(Icons.error, color: Colors.white)),
                        ),
                        DecoratedBox(
                        ...
                        Positioned(
                          ...
                          child: Text(item.caption, style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Colors.white, shadows: [const Shadow(blurRadius: 8)]), textAlign: TextAlign.center),
                        )
                      ],
                    );
                  }
            ...
            Positioned(
              ...
              child: Row(
                children: List.generate(storyItems.length, (index) => Expanded(
                  child: Padding(
